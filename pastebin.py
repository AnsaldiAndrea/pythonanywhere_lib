...
class PastebinAPI(object):
    ...
    _paste_request = '<paste>'.encode('utf-8')

    # String to determine bad API requests
    _bad_request = 'Bad API request'.encode('utf-8')

    _user_request = '<user>'.encode('utf-8')

    # Base domain name
    _base_domain = 'pastebin.com'

    # Valid Pastebin URLs begin with this string (kinda bvious)
    _prefix_url = ('https://%s/' % _base_domain).encode('utf-8')
    ...
    # URL to the Raw paste API (public, unlisted and private)
    ...
    def user_details(self, api_dev_key, api_user_key):
        ...
        elif not response.startswith(self._user_request):
            raise PastebinError(response)
    ...
    def trending(self, api_dev_key):
        ...
        elif not response.startswith(self._paste_request):
            raise PastebinError(response)
    ...
    def generate_user_key(self, api_dev_key, username, password) -> bytes:
        ...
        # lets try to read the URL that we've just built.
        request_string = urllib.request.urlopen(self._api_login_url, urllib.parse.urlencode(argv).encode('utf-8'))
        response = request_string.read()
    ...
    def raw(self, api_dev_key, api_user_key, api_paste_key) -> bytes:
        ...
        Usage Example::
            >>> from pastebin import PastebinAPI
            >>> x = PastebinAPI()
            >>> raw_text = x.raw('453a994e0e2f1efae07f8759e59e075b',
											'c57a18e6c0ae228cd4bd16fe36da381a'
            ...                             's3Xcqmp9')
            >>> print raw_text
        ...
        # option for raw paste
        argv['api_option'] = 'show_paste'

        # lets try to read the URL that we've just built.
        request_string = urllib.request.urlopen(self._api_raw_url, urllib.parse.urlencode(argv).encode('utf-8'))
        response = request_string.read()
